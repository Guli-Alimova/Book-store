let books = [{
    "bookId": 2,
    "title": "HP2",
    "author": "JKR",
    "category": "Mystery",
    "price": 234.00,
    "coverFileName": "9d8f4978-0ef8-42d0-873a-4eb583439237HP2.jpg"
}, {
    "bookId": 3,
    "title": "HP3",
    "author": "JKR",
    "category": "Romance",
    "price": 213.00,
    "coverFileName": "c63ade52-3f90-41fa-980a-1136b6ad2128HP3.jpg"
}, {
    "bookId": 4,
    "title": "HP4",
    "author": "JKR",
    "category": "Fiction",
    "price": 321.00,
    "coverFileName": "9d31690d-3b1d-4faa-a1d2-3a680a935008HP4.jpg"
}, {
    "bookId": 5,
    "title": "HP5",
    "author": "JKR",
    "category": "Mystery",
    "price": 432.00,
    "coverFileName": "8f5f8b64-794c-4dbb-b369-6cc9cc762ce0HP5.jpg"
}, {
    "bookId": 6,
    "title": "HP6",
    "author": "JKR",
    "category": "Fantasy",
    "price": 432.00,
    "coverFileName": "cff3d5ee-71f3-43d8-8625-33abcd48659eHP6.jpg"
}, {
    "bookId": 7,
    "title": "HP7",
    "author": "JKR",
    "category": "Fiction",
    "price": 325.00,
    "coverFileName": "4ec2ffb6-b21a-43ce-bf90-04d56ec72644HP7.jpg"
}, {
    "bookId": 15,
    "title": "HP1",
    "author": "JKR",
    "category": "Fantasy",
    "price": 342.00,
    "coverFileName": "f73b4e21-28fe-4d3f-895b-d8a553b7ba6fHP1.jpg"
}, {
    "bookId": 18,
    "title": "Rot & Ruin",
    "author": "Jonathan Maberry ",
    "category": "Biography",
    "price": 123.00,
    "coverFileName": "eb592aa9-831e-47ed-aea8-fa79d7d316097157310.jpg"
}, {
    "bookId": 21,
    "title": "Slayer",
    "author": " Kiersten White",
    "category": "Mystery",
    "price": 1234.00,
    "coverFileName": "6d91b7b0-b8d1-41ad-a0ef-65e2324fc1b3Slayer.jpg"
}, {
    "bookId": 28,
    "title": "Çürük",
    "author": "Jonathan ",
    "category": "Fiction",
    "price": 1211.00,
    "coverFileName": "cbf832c1-cd01-47b3-93fa-62995a7dca1118799023.jpg"
}, {
    "bookId": 29,
    "title": "Roomies",
    "author": "Christina Lauren ",
    "category": "Biography",
    "price": 334.00,
    "coverFileName": "267e7cea-d66e-4e00-a220-c0ee7e70fdaf33322.jpg"
}, {
    "bookId": 30,
    "title": "A Princess in Theory",
    "author": "Alyssa Cole",
    "category": "Fiction",
    "price": 545.00,
    "coverFileName": "42314d38-bbc8-40ed-b890-612ea45791d735271238.jpg"
}, {
    "bookId": 31,
    "title": "Robbie",
    "author": "Ella Frank",
    "category": "Fiction",
    "price": 345.00,
    "coverFileName": "2141f7a1-e6b8-4d88-91e3-0d3a141117be39086215.jpg"
}, {
    "bookId": 32,
    "title": "Wicked and the Wallflower",
    "author": "Sarah MacLean",
    "category": "Fiction",
    "price": 512.00,
    "coverFileName": "75f8ca57-6b59-43aa-ae05-f51a7429dd5836301036.jpg"
}, {
    "bookId": 33,
    "title": "Dr. Strange Beard",
    "author": "Penny Reid",
    "category": "Biography",
    "price": 312.00,
    "coverFileName": "9749b4d2-d8be-4e9b-b7ea-d437a3a9bf0e136395874.jpg"
}, {
    "bookId": 34,
    "title": "The Simple Wild",
    "author": "K.A. Tucker",
    "category": "Biography",
    "price": 111.00,
    "coverFileName": "b868eb26-f12c-4dcf-ba19-03e0d6cafb8d36373564.jpg"
}, {
    "bookId": 35,
    "title": "Marriage of Inconvenience",
    "author": "Penny Reid",
    "category": "Fantasy",
    "price": 345.00,
    "coverFileName": "8d3a0352-270b-405d-b8bc-ee0f59179cbf36395764.jpg"
}, {
    "bookId": 36,
    "title": "The Hookup",
    "author": "Kristen Ashley ",
    "category": "Romance",
    "price": 434.00,
    "coverFileName": "dc785e9f-a753-42f7-a87e-fda5e97ce67bqq.jpg"
}, {
    "bookId": 37,
    "title": "Birthday Girl",
    "author": "Penelope Douglas ",
    "category": "Romance",
    "price": 441.00,
    "coverFileName": "7616f5e9-a42a-4268-8ff0-4f77547dcc9crr.jpg"
}, {
    "bookId": 38,
    "title": "Red Rising",
    "author": "Pierce Brown",
    "category": "Romance",
    "price": 512.00,
    "coverFileName": "24cd89a7-737f-490e-8c8e-cbeac7eac054tt.jpg"
}, {
    "bookId": 39,
    "title": "Before We Were Yours",
    "author": "Lisa Wingate ",
    "category": "Romance",
    "price": 123.00,
    "coverFileName": "15396272-e3ec-4b1c-97d0-d2e841898f55aa.jpg"
}, {
    "bookId": 40,
    "title": "A Dance with Dragons",
    "author": "George R.R. Martin",
    "category": "Fantasy",
    "price": 412.00,
    "coverFileName": "63df0288-6589-4e4c-8a68-def0bbf72714qa.jpg"
}, {
    "bookId": 41,
    "title": "The Hate U Give",
    "author": "Angie Thomas",
    "category": "Biography",
    "price": 122.00,
    "coverFileName": "3d894fa1-8746-4443-b244-99624cc39f1fq1we.jpg"
}, {
    "bookId": 42,
    "title": "All the Light We Cannot See",
    "author": "Anthony Doerr ",
    "category": "Romance",
    "price": 542.00,
    "coverFileName": "e49d1ff8-3bad-49fc-b1f1-496fc7fef07aq2.jpg"
}, {
    "bookId": 43,
    "title": "The Help",
    "author": "Kathryn Stockett",
    "category": "Biography",
    "price": 124.00,
    "coverFileName": "921ff624-bcdb-4b48-8e5b-c3f34078bd11q3.jpg"
}, {
    "bookId": 44,
    "title": "A Court of Mist and Fury",
    "author": "Sarah J. Maas ",
    "category": "Romance",
    "price": 645.00,
    "coverFileName": "7996d691-beb6-4c41-82f0-4a3f23641db626073150.jpg"
}, {
    "bookId": 45,
    "title": "The Martian",
    "author": "Andy Weir",
    "category": "Fantasy",
    "price": 321.00,
    "coverFileName": "5b7162d6-2780-461b-be6f-e4debac083ad18007564.jpg"
}, {
    "bookId": 46,
    "title": "Catching Fire",
    "author": "Suzanne Collins",
    "category": "Mystery",
    "price": 658.00,
    "coverFileName": "6df4a30a-e8a1-467d-9da6-d826989a9c606148028.jpg"
}, {
    "bookId": 47,
    "title": "The Fault in Our Stars",
    "author": " John Green ",
    "category": "Mystery",
    "price": 346.00,
    "coverFileName": "a99a994c-b5d5-4f17-807f-33bcd1003505q3333.jpg"
}, {
    "bookId": 48,
    "title": "11/22/63",
    "author": "Stephen King",
    "category": "Mystery",
    "price": 556.00,
    "coverFileName": "ab486162-5053-46ec-a89a-fdf2b686ff6310644930.jpg"
}, {
    "bookId": 72,
    "title": "City of Girls",
    "author": "Elizabeth Gilbert ",
    "category": "Fiction",
    "price": 1222.00,
    "coverFileName": "e94cf1e2-a7bf-47e1-aed8-b8931aeee6b342135029.jpg"
}, {
    "bookId": 73,
    "title": "Mrs. Everything",
    "author": "Jennifer Weiner ",
    "category": "Fiction",
    "price": 4444.00,
    "coverFileName": "07492ea6-09fb-46e9-8c9b-8a8f51afe989qq.jpg"
}, {
    "bookId": 74,
    "title": "Magic for Liars",
    "author": "Sarah Gailey ",
    "category": "Fiction",
    "price": 777.00,
    "coverFileName": "7a904907-be5d-42c3-af15-6b770e5bd0d0qq.jpg"
}, {
    "bookId": 75,
    "title": "Fix Her Up",
    "author": "Tessa Bailey ",
    "category": "Romance",
    "price": 3333.00,
    "coverFileName": "1561f3e0-c461-4273-864c-9e67c620a490qq.jpg"
}, {
    "bookId": 76,
    "title": "Like a Love Story",
    "author": "Abdi Nazemian ",
    "category": "Romance",
    "price": 6666.00,
    "coverFileName": "bb4382d6-4c3f-462f-bbbe-a7f88afc1436qq.jpg"
}, {
    "bookId": 77,
    "title": "The Beholder",
    "author": "Anna Bright ",
    "category": "Romance",
    "price": 3333.00,
    "coverFileName": "5ba1968d-a7f5-4a04-99c2-281088b8532fqq.jpg"
}, {
    "bookId": 78,
    "title": "The Last Pirate of New York: A Ghost Ship, a Killer, and the Birth of a Gangster Nation",
    "author": "Rich Cohen",
    "category": "Biography",
    "price": 1212.00,
    "coverFileName": "716d7202-d166-452a-b998-a17fe2666438qq.jpg"
}, {
    "bookId": 79,
    "title": "The Ministry of Truth: The Biography of George Orwell's \"1984\"",
    "author": "Dorian Lynskey",
    "category": "Biography",
    "price": 5874.00,
    "coverFileName": "0e6688cb-47e4-438c-9648-543f174878d5qq.jpg"
}, {
    "bookId": 80,
    "title": "Rough Magic: Riding the World's Loneliest Horse Race",
    "author": "Lara Prior-Palmer",
    "category": "Biography",
    "price": 8264.00,
    "coverFileName": "577f11d3-df70-4671-a468-6318da8ec16cqq.jpg"
}, {
    "bookId": 81,
    "title": "This Storm",
    "author": "James Ellroy",
    "category": "Mystery",
    "price": 2232.00,
    "coverFileName": "d810fa08-0ed8-4dcb-8158-6f1ab98a45d3qq.jpg"
}, {
    "bookId": 83,
    "title": "One Night at the Lake",
    "author": " Bethany Chase",
    "category": "Mystery",
    "price": 555.00,
    "coverFileName": "fe476d1e-cdc3-404b-913e-a52889d5b9fcqq.jpg"
}, {
    "bookId": 84,
    "title": "The Chosen",
    "author": "Taran Matharu ",
    "category": "Fantasy",
    "price": 5555.00,
    "coverFileName": "ffa6e013-86ac-4d00-8516-a8fd8bc8ddf5qq.jpg"
}, {
    "bookId": 85,
    "title": "All of Us with Wings",
    "author": "Michelle Ruiz Keil ",
    "category": "Fantasy",
    "price": 555.00,
    "coverFileName": "3618116f-ba09-4172-bc6a-7aa2dbe7d7c4qq.jpg"
}, {
    "bookId": 86,
    "title": "Soul of the Sword",
    "author": " Julie Kagawa",
    "category": "Fantasy",
    "price": 55555.00,
    "coverFileName": "58dbce6c-7c82-4491-be4f-c170867ba4b6qq.jpg"
}, {
    "bookId": 90,
    "title": "7/1/1993",
    "author": "qwert",
    "category": "Fiction",
    "price": 123.00,
    "coverFileName": "Default_image.jpg"
}]
//  shu yerda jasondan olib kochirib qoyiladimi doim
//  console.log(books.length);

//  birinchi ish

const booksEl = document.getElementById('books');

// ikkinchi ish 

function createBookDiv(bookObject) {
    let divEl = document.createElement('div');
    divEl.className = 'book';
    divEl.innerHTML = `
    <img src="img/${bookObject.coverFileName}" alt="${bookObject.title}" class="book__photo">
    <h3 class="book__title">${bookObject.title}</h3>
    <h4 class="book__category">${bookObject.category}</h4>
    <h5 class="book__author">${bookObject.author}</h5>
    <div class="book__price">${bookObject.price}</div>
    <button type="button" data-id="${bookObject.bookId}" class="book__btn">Add to Cart</button>
  
    `;
    return divEl;

}
// uchinchi ish

function appendBooks() {
    books.forEach((el) => {
        let bookdiv = createBookDiv(el);
        booksEl.appendChild(bookdiv);
    })
}
if (booksEl) appendBooks();

// tortinchi ish

function filterBooksByCat(cat) {
    let result = books.filter((el) => {
        return el.category == cat;

    });

    return result;
}


//  beshinchi ish


const catButtonsEl = document.getElementsByClassName('category-buttons')[0];
if (catButtonsEl) {
    catButtonsEl.addEventListener('click', function (event) {
        if (event.target.className == 'category-btn');
        let selectedCat = event.target.innerText; // bosilgan element(event.target)
        let selectedBooks;
        if (selectedCat !== 'All') {
            selectedBooks = filterBooksByCat(selectedCat)
        } else {
            selectedBooks = books;
        }

        booksEl.innerText = '';
        appendSelectedBooks(selectedBooks)
    });
}



// oltinchi ish

function appendSelectedBooks(sbooks) {
    sbooks.forEach((el) => {
        let bookdiv = createBookDiv(el);
        booksEl.appendChild(bookdiv);
    });
}

// yettinchi ish

function searchBook(searchText) {
    let resultArr = books.filter((el) => {
        return el.title.includes(searchText) || el.author.includes(searchText);

    });
    return resultArr;

}
searchBook('simple')

//  sakkizinchi ish

const searchInput = document.getElementById('search');

searchInput.addEventListener('keyup', function (event) {
    let searchedbooks = searchBook(searchInput.value);


    booksEl.innerHTML = ' ';
    appendSelectedBooks(searchedbooks);
});

// to'qqizinchi ish
let myCart = cartItemsFromLocalStorage();

function addBookToCart(bookId) {
    if (!myCart.includes(bookId)) {
        myCart.push(bookId);
    }
    console.log(myCart);


}


// o'ninchi ish
if (booksEl) {
    booksEl.addEventListener('click', function (event) {
        if (event.target.className == 'book__btn') {
            let idOfCurrentBook = +event.target.getAttribute('data-id');
            addBookToCart(idOfCurrentBook)
            showCartCount();
            cartItemsToLocalStorage();
        }
    });
}


// o'n birinchi ish
const countSpan = document.querySelector('.cart-count');

function showCartCount() {
    countSpan.innerText = myCart.length;
}

// o'n ikkinchi ish
showCartCount();

function cartItemsToLocalStorage() {
    localStorage.setItem('meningkarzinam', JSON.stringify(myCart));
}

// on uchinchi ish


function cartItemsFromLocalStorage() {
    let result = JSON.parse(localStorage.getItem('meningkarzinam'));
    return result ? result : [];
}

//    on tortinchi ish
// "bookId": 2,
//     "title": "HP2",
//     "author": "JKR",
//     "category": "Mystery",
//     "price": 234.00,
//     "coverFileName": "9d8f4978-0ef8-42d0-873a-4eb583439237HP2.jpg"



function createCartItemTr(el) {
    let trEl = document.createElement('tr');
    trEl.innerHTML = `
     
     <td class="images"><img src="${el.coverFileName}" alt="${el.title}"></td>
     <td class="title">${el.title}</td>
     <td class="price">${el.price}</td>
     <td>
         <button type="button" class="minus">-</button>
         <span class="quantity">${1}</span>
         <button type="button" class="plus">+</button>
     </td>
     <td class="total_price">${el.price}</td>
     <td><button type="button" data-id = "${el.bookId}">Delete</button></td>
     `;
     return trEl;
}

//   on beshinchi ish

const cartItemsEl = document.getElementById('cartItems')

if (cartItemsEl){
   let meningKarzinamArr =  cartItemsFromLocalStorage();
   meningKarzinamArr.forEach((el)=> {
       let book = books.filter((item) => {
           return item.bookId == el

       });
    let element = createCartItemTr(book[0]);
       cartItemsEl.appendChild(element);     
   });

}
